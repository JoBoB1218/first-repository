...
---
마크다운을 처음 써보는 거라 익숙하지가 않아서 힘들었던것 같다. 그러나 점점 많은 곳에 사용되고 필요하다는 것을 느끼며 좀 더 연습해서 사용법을 익혀야 겠다는 생각이 들었다.   
최전선에 있는 인공지능 개발자들이 인공지능의 발전에 대해 경각심을 가지고 있다는 점이 흥미로웠다. 아직은 이 분야에 대해서는 깊게는 모르지만, 나도 인공지능과 관련된 윤리에 대해서는 개발하면서 항상 생각해야겠다.
